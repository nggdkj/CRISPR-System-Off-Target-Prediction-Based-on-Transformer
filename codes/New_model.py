...
from tensorflow.keras.layers import Attention, Dense, Conv2D, Bidirectional, LSTM, Flatten, Input, Activation, Reshape, Dropout, Concatenate, AveragePooling1D, MaxPool1D, BatchNormalization, Attention, GlobalAveragePooling1D, GlobalMaxPool1D, GRU, AdditiveAttention, AlphaDropout, LeakyReLU, MultiHeadAttention, LayerNormalization
...
def new_model(xtrain, ytrain, xtest, ytest, input_shape, num_classes, batch_size, epochs, callbacks, saved_prefix, retrain=False):
    if retrain or not os.path.exists('{}+new_model.h5'.format(saved_prefix)):
        ...
        # modified
        attention_1_output = MultiHeadAttention(num_heads=4, key_dim=64)(bidirectional_1_output, bidirectional_1_output)
        attention_1_output = MultiHeadAttention(num_heads=2, key_dim=64)(attention_1_output, attention_1_output)
        ...
        model.save('{}+new_model.h5'.format(saved_prefix))
    model = load_model('{}+new_model.h5'.format(saved_prefix))
    return model